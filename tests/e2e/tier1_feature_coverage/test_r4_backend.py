"""
E2E tests for Tier 1: Feature Coverage (R4 - FastAPI Backend & DB).
File: tests/e2e/tier1_feature_coverage/test_r4_backend.py
"""
import os
import sqlite3
import pytest
...
def test_fifo_queue_sequential_execution(api_client: TestClient):
    ...
    # Should be pending while job1 is processing or completed
...
def test_sse_log_stream_endpoint(api_client: TestClient):
    ...
    # Read stream logs
    logs = []
    with api_client.get(f"/api/jobs/{job_id}/logs/stream", stream=True) as stream:
