"""
Proposed E2E tests for Tier 3: Cross-Feature Integration.
...
@pytest.mark.asyncio
async def test_alpha_webm_drive_upload_flow():
    ...
    async with httpx.AsyncClient(**client_kwargs, base_url="http://test") as client:
        ...
        down_resp = await client.get(f"/api/jobs/{job_id}/download?filename=output_alpha.webm")
        assert down_resp.status_code == 200
        assert down_resp.content == b"MOCK_WEBM_VP9_ALPHA_DATA"

        client.cookies.set("session_token", "mock_valid_token")
        upload_resp = await client.post(f"/api/jobs/{job_id}/upload-drive", data={"chunk_index": 0, "total_chunks": 1})
